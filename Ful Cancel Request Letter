			...
			<body>
              ...
              <br />
							...
							<xsl:if test="notification_data/request/cancel_reason != ''">
								<tr>
									<td>
										<h3><b> @@request_cancellation_note@@: </b> <!-- Remove the Variable Name around the XSL value, and then the p tags to revert back. -->
									<xsl:value-of select="notification_data/request/cancel_reason" /></h3>
									</td>
								</tr>
							</xsl:if>
						</table>
				...
				<xsl:call-template name="lastFooter" /> <!-- footer.xsl --> <xsl:call-template name="myAccount" /> 
